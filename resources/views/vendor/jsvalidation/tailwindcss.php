<script type="module">
    jQuery(document).ready(function(){
        ...
        $("<?= $validator['selector']; ?>").on('submit', function (){
            $("<?= $validator['selector']; ?>").find(':submit').html(btnText.replace("Remove", ""));

            $("<?= $validator['selector']; ?>").find(':submit').prop('disabled', true);
            $("<?= $validator['selector']; ?>").find(':submit').prepend('<i data-loading-icon="oval" data-color="white" class="w-5 h-5 pr-1 mx-auto"></i>');
            tailwind.svgLoader()
            helper.delay(1500)
        });
        ...
        $("<?= $validator['selector']; ?>").each(function() {
            ...
            $(this).validate({
                ...
                highlight: function (element) {
                    $(element).removeClass('border-success').addClass('border-danger'); // add the Bootstrap error class to the control group
                ...
                success: function (element) {
                    $(element).removeClass('border-danger').addClass('border-success'); // remove the Boostrap error class from the control group
                    parent = $(element).parent();
                    // parent.find('label.form-label').removeClass('text-danger').addClass('text-success');
                },
